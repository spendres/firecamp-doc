...
## URL Field
The WebSockets endpoint can be specified in the URL field. This is the endpoint on which the connection request will be made to the server when the `Send` button is clicked.

## Send Button (Execute)
The orange `Send` button in the URL bar execute/sends the connection request to the endpoint specified in the URL field with the parameters you configured.
